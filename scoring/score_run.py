...
def main(score_paths):
    score_paths.sort()
    print('Time\tDCF\tFNR\tFPR\tIMLM\t#bootstrap\t#AL\t#FP\t#FN\t#Target\t#Nontarget\t#CF retained\t% target in adapt pool\ttot labels/tot samples')
    for score_path in score_paths:
        ...
        with open(score_file.replace('scores.csv', 'time.txt'), 'r') as f:
            tm = f.read()
        tm = float(tm.strip())
        print(f'{tm:.1f}\t{dcf:.4f}\t{fnr:.4f}\t{fpr:.4f}\t{imlm:.4f}\t{n_boot:d}\t\t{n_al:d}\t{fps:d}\t{fns:d}\t{ps:d}\t{ns:d}\t\t{n_cf:d}\t\t{perc_targ:.2f}\t\t\t{p_samples:.4f}')
