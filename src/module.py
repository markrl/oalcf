...
class VtdModule(LightningModule):
    ...
    def on_validation_epoch_end(self):
        fnr = self.val_fns/self.val_ps if self.val_ps > 0 else 0
        fpr = self.val_fps/self.val_ns if self.val_ns > 0 else 0
    ...
    def on_test_epoch_end(self):
        fnr = self.test_fns/self.test_ps if self.test_ps > 0 else 0
        fpr = self.test_fps/self.test_ns if self.test_ns > 0 else 0
        dcf = 0.25*fpr + 0.75*fnr
